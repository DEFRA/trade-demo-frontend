...
export const transportDetailsController = {
  get: {
    handler(request, h) {
      const sessionData = {
        'transport-selection': getSessionValue(request, 'transport-selection')
      }
  ...
  post: {
    async handler(request, h) {
      const { bcp } = request.payload

      // Simple validation
      if (!bcp || bcp.trim().length === 0) {
        const sessionData = { bcp: bcp }
        const error = {
          details: [
            {
              path: ['bcp'],
      ...
      // Store in session
      setSessionValue(request, 'transport-selection', bcp.trim())
